...
public class PlanDetailsController extends DbHelper {
    ...
    public ArrayList<HashMap<String, String>> getMonthDetails(int cycle_month) {
        ...
        while (cur.moveToNext()) {
            String date = cur.getString(cur.getColumnIndex("cycle_day"));
            String sql2 = "SELECT COUNT(pd.id) as count, (SELECT COUNT(c.id) FROM Calls as c WHERE c.status_id = 2) as actual_calls FROM PlanDetails as pd " +
                    "LEFT JOIN Calls as c ON pd.plan_details_id = c.plan_details_id WHERE pd.cycle_day = '" + date + "'";
            Cursor cur2 = db.rawQuery(sql2, null);
            HashMap<String, String> map = new HashMap<>();
            map.put("date", date);

            if (cur2.moveToNext()) {
                map.put("divisor", cur2.getString(cur2.getColumnIndex("count")));
                map.put("dividend", cur2.getString(cur2.getColumnIndex("actual_calls")));
            }
